...
do
  aws s3 --profile xiaoqi cp ${file} "s3://prescriberpipelineproj/dimension_city/$bucket_subdir_name/"
  echo "City File $file is pushed to S3."
done

for file in ${LOCAL_FACT_DIR}/*.*
do
  aws s3 --profile xiaoqi cp ${file} "s3://prescriberpipelineproj/presc/$bucket_subdir_name/"
