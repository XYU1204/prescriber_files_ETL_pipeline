...
def perform_data_clean(df1, df2):
    ...
    # 7 Check and clean all the Null/Nan Values
        info = df_fact_sel.select([count(when(isnan(c) | col(c).isNull(), c)).alias(c) for c in df_fact_sel.columns])
        logger.info(f"The number of Null values in fact dimension dataFrame has:.")
        logger.info('\n \t' + info.toPandas().to_string(index=False))
    ...
    # 10 Impute TRX_CNT where it is null as avg of trx_cnt for the prescriber
        ...
        logger.error("Error in the method - spark_curr_date(). Please check the Stack Trace. "
